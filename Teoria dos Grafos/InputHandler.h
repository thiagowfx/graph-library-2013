...
public:
    ...
    void removeDuplicates(const char *inputFile, const char *outputFile, bool weighted);
    /** Remove as linhas duplicadas consecutivas do arquivo <i>inputFile</i>, escrevendo a saída no arquivo <i>outputFile</i>. */
    void removeDuplicates2(const char *inputFile, const char *outputFile, bool weighted);
    /** Retorna um vetor com as <i>n</i> maiores arestas do grafo do arquivo <i>filename</i>. */
    std::vector<double> greatestEdges(unsigned long long n, const char *filename, bool weighted);
private:
};
